        ...
        for (Map.Entry<String, FlamesCommand> entry: Flames.commandMap.entrySet()) {
            ...
            if (entry.getKey().contains(event.getName())) {
                try {
                    ...
                    event.reply("Hey bestie, looks like you've opted not to have your data collected by Flames. You can't have your cake, but then not eat it, so would you mind eating the cake first? Thanks!");
                }
            }
        }
    }
